...
export class Signer implements SignerInterface {
  ...
  recoverPublicKeys(
    ...
    const block_header = root.lookupType("koinos.protocol.block_header");
    ...
    if (block.signature) {
      ...
      const message = block_header.create(headerDecoded);
      headerBytes = block_header.encode(message).finish();
    } else {
      ...
      const message = transaction_header.create(headerDecoded);
      headerBytes = transaction_header.encode(message).finish();
    }
  ...
  async prepareTransaction(tx: TransactionJson): Promise<TransactionJson> {
    ...
    const operation = root.lookupType("koinos.protocol.operation");
    ...
    if (tx.operations) {
      for (let index = 0; index < tx.operations?.length; index += 1) {
        ...
        const message = operation.create(operationDecoded);
        const operationEncoded = operation
          .encode(message)
    ...
    const message = transaction_header.create(headerDecoded);
    const headerBytes = transaction_header
      .encode(message)
      .finish() as Uint8Array;
  ...
  async prepareBlock(block: BlockJson): Promise<BlockJson> {
    ...
    const headerDecoded = btypeDecode(block.header, btypeBlockHeader!, false);
    const message = block_header.create(headerDecoded);
    const headerBytes = block_header.encode(message).finish() as Uint8Array;
