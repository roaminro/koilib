...
export class Provider {
  ...
  async getNonce(
    ...
    const message = value_type.decode(valueBuffer);

    const object = value_type.toObject(message, {
      longs: String,
      defaults: true,
  ...
  async getNextNonce(account: string): Promise<string> {
    ...
    const nonceEncoded = value_type.encode(message).finish() as Uint8Array;
  ...
  async sendTransaction(
    ...
    (transaction as TransactionJsonWait).wait = (
      type: "byTransactionId" | "byBlock" = "byBlock",
      timeout = 15000
    ) => {
